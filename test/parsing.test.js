...
it('text outside the root node will throw an error', () => {
	expect(() => sync(`skeet<x />skrrrr`)).toThrow('text data outside of root node');
});

it('whitespace outside the root node will notthrow an error', () => {
	expect(() => sync(`<?xml version="1.0"?>     <x />`)).not.toThrow();
});

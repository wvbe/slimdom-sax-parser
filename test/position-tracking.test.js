...
it('doc types', () => {
	...
	expect(stringForPosition(xml, doc.firstChild.position)).toBe(
...
it('processing instructions', () => {
	...
	const doc = sync(xml, { position: true });
	expect(stringForPosition(xml, evaluateXPath('/processing-instruction()', doc).position)).toBe(
		`<?pi-target pi-data?>`
	);
	...
	expect(stringForPosition(xml2, evaluateXPath('/processing-instruction()', doc2).position)).toBe(
		`<?pi-tarssdgsget    ?>`
	);
});
...
it('comments', () => {
	...
	expect(stringForPosition(xml, evaluateXPath('/comment()[1]', doc).position)).toBe(`<!-- comment -->`);
	expect(stringForPosition(xml, evaluateXPath('/comment()[2]', doc).position)).toBe(`<!--
			a multi-line comment
		-->`);
	...
	expect(stringForPosition(xml, evaluateXPath('/element()', doc).position)).toBe(`<x />`);
});
...
it('elements', () => {
	...
	const doc = sync(xml, { position: true });
	expect(stringForPosition(xml, doc.documentElement.position)).toBe(`<root-node nerf="derp"
		sk="z">`);
	expect(stringForPosition(xml, doc.documentElement.firstChild.position)).toBe(`<self-closing unit="" />`);

	// This would fail if the closing tags of elements were not tracked in onCloseTag
	const xml2 = `<x><a></a><b /><c></c></x>`;
	const doc2 = sync(xml2, { position: true });
	expect(stringForPosition(xml2, evaluateXPath('/*/a', doc2).position)).toBe(`<a>`);
	expect(stringForPosition(xml2, evaluateXPath('/*/c', doc2).position)).toBe(`<c>`);
	expect(stringForPosition(xml2, evaluateXPath('/*/b', doc2).position)).toBe(`<b />`);
});
...
it('Various things in a messy XML file', () => {
	...
	expect(stringForPosition(xml, evaluateXPath('/comment()', doc).position)).toBe(`<!-- multi-line
		comment -->`);

	expect(stringForPosition(xml, evaluateXPath('/*', doc).position)).toBe(`<root attr="val">`);

	expect(stringForPosition(xml, evaluateXPath('/*/*[2]', doc).position)).toBe(`<a:root xmlns="http://default"
			xmlns:a="http://a"
			xmlns:b="http://b"
			xmlns:d="http://d"
			a:attr="A"  b:attr="B" attr="def">`);

	expect(stringForPosition(xml, evaluateXPath('//Q{http://a}root/text()[2]', doc).position)).toBe(`
			snapje
			`);
});
